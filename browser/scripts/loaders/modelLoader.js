(function() {
	...
	function ModelLoader(url) {
		...
		console.log("This is entering ModelLoader: "+ url);
		if (typeof(url) == 'object'){  //check if array skip switch statement
			this.loadObj(url);
		}
		else {
	...
	ModelLoader.prototype.loadObj = function(urls) {
		...
		if (Array.isArray(urls)){  //if missing mtl
			if (urls.length <2) {
				var objUrl = urls[0];
				var mtlUrl = urls[1].replace('.obj', '.mtl')
			}
			else{
				objUrl = urls[0];
				mtlUrl = urls[1]
			...
			//it's a string
			var objUrl = urls;
			var mtlUrl = urls.replace('.obj', '.mtl')
		}
